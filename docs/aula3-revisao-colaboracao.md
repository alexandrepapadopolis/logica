...
O GitHub evoluiu de um simples ecossistema de hospedagem de código para uma plataforma completa de DevOps e colaboração. Para operar em nível profissional, engenheiros de software não apenas utilizam comandos Git, mas seguem fluxos de trabalho estruturados que garantem a estabilidade do código, a segurança da informação e a eficiência do time.
...
A branch principal (`main` ou `master`) representa o código em estado de produção (estável e pronto para o usuário). Nunca se deve commitar diretamente nela. 
* Ao iniciar uma tarefa, cria-se uma branch a partir da `main` atualizada.
* **Padrão de Nomenclatura:** Adote prefixos descritivos para organizar o repositório:
  ...
  * `docs/atualiza-readme`: For exclusividade de documentação.
...
* Faça commits a cada unidade lógica de trabalho concluída (ex: criar a interface de um botão, depois criar a validação do clique, depois integrar com a API).

### Passo 3: Criação do Pull Request (PR) como Ferramenta de Comunicação
O Pull Request não é apenas um botão de "mesclar", mas uma ferramenta de comunicação síncrona/assíncrona. Ao abrir um PR, o autor deve preencher um modelo (template) contendo:
* **Descrição:** O que foi feito e o porquê.
* **Contexto:** Link para a tarefa correspondente (Jira, Trello ou GitHub Issues).
* **Instruções de Teste:** O passo a passo para o revisor rodar e validar o código localmente.
* **Evidências:** Capturas de tela ou GIFs mostrando a alteração visual (essencial para Frontend).

### Passo 4: Code Review (Revisão de Código) e Revisores Obrigatórios
...
* **Segurança:** Ausência de chaves de API expostas ou vulnerabilidades.
* **Cobertura de testes:** Se foram inclusos testes unitários ou de integração.

### Passo 5: CI/CD (Integração e Entrega Contínuas)
O GitHub possui o *GitHub Actions*, que roda testes automatizados assim que o PR é aberto. Se os testes quebrarem, o PR fica bloqueado para Merge, impedindo que código defeituoso seja integrado à `main`.
...
1. **Merge Commit:** Mantém todo o histórico de commits da branch de forma cronológica. Cria um "commit de merge".
2. **Squash and Merge:** Amassa todos os commits da sua branch de trabalho em um único commit limpo antes de enviar para a `main`. Ideal para manter o histórico da `main` linear e limpo.
...
Dependendo do modelo de governança do software (proprietário ou open-source), a colaboração se divide em dois formatos:
...
* **Fluxo:** 
  1. O desenvolvedor faz um **Fork** (gera uma cópia completa e independente do projeto para a sua própria conta do GitHub).
...
Mensagens de commit ruins destroem a rastreabilidade do projeto. Adote a convenção internacional de Commits Semânticos:
...
* `docs`: Alterações exclusivamente na documentação (ex: README.md).
...
* **Regra de ouro:** Resolva o conflito sempre na sua branch de trabalho, nunca na `main`.
...
### 🌟 Uso Inteligente de Issues e Projetos (GitHub Projects)
O GitHub possui ferramentas nativas de gerenciamento de projetos baseadas em Kanban:
* **Issues (Problemas/Tarefas):** Devem relatar bugs encontrados por usuários ou documentar novas funcionalidades requisitadas. Cada Issue recebe etiquetas (labels) como `bug`, `enhancement`, `help wanted`.
...
Nunca suba para o GitHub arquivos de configuração local, pastas de dependências (como `node_modules` no ecossistema JavaScript) ou credenciais de banco de dados.
...
| :--- | :--- | :--- | :--- |
| **Merge Commit tradicional** | Preserva a história real e cronológica de cada ramificação. | Cria um histórico visual poluído ("gráfico de metrô"). | Equipes pequenas com fluxos de ramificação simples. |
| **Squash and Merge** | Mantém a branch principal extremamente limpa com um commit por tarefa. | Perde os detalhes do micro-histórico de desenvolvimento. | Repositórios comerciais de ritmo acelerado e muitos PRs. |
| **Rebase and Merge** | Histórico 100% linear, facilitando o rastreamento com `git blame`. | Altera o histórico de commits, exigindo cuidado ao lidar com branches públicas. | Equipes sêniores que prezam por uma linha do tempo impecável. |
